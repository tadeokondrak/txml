...
pub struct Attrs<'a> {
    // Invariant: No leading or trailing whitespace
    text: &'a str,
}
...
impl Error {
    ...
    /// invalid quote character found while processing attribute
    pub const ATTR_INVALID_QUOTE: Error =
        Error("invalid quote character found while processing attribute");
    /// found '&' but no terminating ';' in text
    pub const UNTERMINATED_ENTITY: Error = Error("found '&' but no terminating ';' in text");
                ...
                if s.starts_with('&') {
                    ...
                    *s = &s[semi + 1..];
                    match esc {
                        ...
                        "quot" => Some(Ok('"')),
                        s if s.starts_with('#') => {
                            let (s, radix) = match s[1..].strip_prefix('x') {
                                Some(s) => (s, 16),
                                None => (&s[1..], 10),
                            };
                            const ERR: Error = Error::INVALID_NUMERIC_ENTITY;
                            let c = u32::from_str_radix(s, radix)
                                .map_err(|_| ERR)
                                .and_then(|n| n.try_into().map_err(|_| ERR));
                            Some(c)
                        }
                        _ => return Some(Err(Error::INVALID_NAMED_ENTITY)),
                    }
                } else {
                    let mut it = s.chars();
                    let c = it.next()?;
...
impl<'a> PartialEq<str> for Text<'a> {
    fn eq(&self, other: &str) -> bool {
        self.clone().zip(other.chars()).all(|(a, b)| a == Ok(b))
    }
}
...
impl<'a> Iterator for Parser<'a> {
    ...
    fn next(&mut self) -> Option<Result<Event<'a>, Error>> {
        if let Some(tag) = self.self_closing.take() {
            ...
            Some(Ok(Event::Pi(self.consume_to("?>")?)))
            ...
            Some(Ok(Event::Comment(self.consume_to("-->")?)))
        } else if self.consume("<![CDATA[") {
            Some(Ok(Event::Text(Text::Verbatim(self.consume_to("]]>")?))))
        } else if self.consume("</") {
            Some(Ok(Event::Close(
                self.consume_to(">")?.trim_matches(WHITESPACE),
            )))
